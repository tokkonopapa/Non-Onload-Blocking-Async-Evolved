...
function getDelay(url) {
	if (url.match(url_parser)) {
		...
		if (wait) {
			return 'http://tokkono.cute.coocan.jp/blog/slow/wp-content/uploads/test/sleep.php?type=img&cache=false&wait=' + wait + '&url=';
		}
	}
	return '';
}
...
function setDelay(url, id) {
	...
	if (id && url.match(url_parser)) {
		...
		// update href
		addDelay(document.getElementById('delay'));
	}
}
